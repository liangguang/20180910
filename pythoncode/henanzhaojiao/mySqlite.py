...
def create_table_test():
    '''创建数据库表测试'''
    print('创建数据库表测试...')
    create_table_sql = '''CREATE TABLE `zpinfo` (
                          `pushtime` varchar(20) NOT NULL,
                          `city` varchar(20) NOT NULL,
                          `title` varchar(200) DEFAULT NULL,
                          `address` varchar(200) DEFAULT NULL,
                          `emailTo` varchar(20) DEFAULT NULL,
                           PRIMARY KEY (`pushtime`,`city`)
                        )'''
    conn = get_conn(DB_FILE_PATH)
    create_table(conn, create_table_sql)

def charge_create_table_test():
    charge_table_sql = 'SELECT count(*) FROM sqlite_master where name=' + '\'zpinfo\''
    create_table_sql = '''CREATE TABLE `zpinfo` (
                         `pushtime` varchar(20) NOT NULL,
                          `city` varchar(20) NOT NULL,
                          `title` varchar(200) DEFAULT NULL,
